...
4. Run `slidex workbench start --deck-id <deck_id>` from the workspace root, passing any extracted fields with the matching `--initial-request`, `--title`, `--audience`, `--decision-goal`, `--source-notes`, `--key-messages`, `--required-claims`, `--constraints`, and `--output-expectations` flags when available.
