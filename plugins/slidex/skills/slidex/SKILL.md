...
For a new deck, use `slidex-start` first. This is mandatory: the local Workbench
must be displayed before generation proceeds. The Workbench saves initial
...
applies a verified update before opening the Workbench. If the response reports
...
return the same Workbench startup response, including `agentBrowserInstruction`
when configured.
