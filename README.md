...
`brief.md`를 작성합니다. 여러 덱이 있으면 프롬프트 실행 전에 대상 덱을
명시해야 합니다.
...
3. 필요하면 덱별 `brand/`, `assets/`, `data/`, `source/` 자료를 추가합니다.
4. `prompts/00_intake_and_strategy.md`로 `${OUT_DIR}/strategy.md`를 생성합니다.
5. `prompts/01_create_deck_spec.md`로 `${OUT_DIR}/deck_spec.json`을 만듭니다.
6. `prompts/02_build_pptx.md`로 편집 가능한 `${OUT_DIR}/final_deck.pptx`를 만듭니다.
7. `prompts/03_visual_qa.md`로 슬라이드를 이미지로 렌더링하고 QA 몽타주와 리포트를 만듭니다.
8. `prompts/04_revise_deck.md`로 문제를 수정하고 다시 렌더링합니다.
9. `prompts/05_finalize_delivery.md`로 최종 산출물을 확인하고 납품 요약을 작성합니다.
