...
```powershell
slidex workbench evidence --deck-id customer-retention --inspector "<name-or-role>" --surface codex_app_in_app_browser --invocation "@slidex create a deck called customer-retention" --thread-id "<codex-app-thread-id-if-visible>" --url "<workbench.url>" --screenshot "<path-to-codex-browser-screenshot.png>" --workbench-visible --saved-input-verified
```
...
version을 기록하거나 로컬에 vendoring하고 SHA-256을 남깁니다. CI runner 제공
Chrome/Chromium은 예외적으로 runner image에서 가져오되, `doctor --render`와
`render_manifest.json`에 실제 exact browser version을 기록해 증거로 남깁니다.
...
- Chrome/Chromium 탐색은 `CHROME_BIN`, `GOOGLE_CHROME_BIN`, `CHROMIUM_BIN`,
  `MSEDGE_BIN`, `CHROME_FOR_TESTING_BIN`, `PLAYWRIGHT_CHROMIUM_BIN`,
  `PLAYWRIGHT_CHROME_BIN`, `PUPPETEER_EXECUTABLE_PATH`를 먼저 보고, 이후 Windows
  Chrome/Edge 설치 경로, macOS app bundle, Linux PATH 명령명, Playwright/Puppeteer/
  Chrome-for-Testing cache 후보를 자동 탐색합니다.
- `slidex codex app-server start`는 Linux/macOS에서 Unix socket을 우선 사용하고,
  OS socket path 제한을 넘으면 `127.0.0.1` loopback WebSocket으로 폴백합니다.
  Windows는 `127.0.0.1` loopback WebSocket을 기본 transport로 사용합니다. 모든 OS에서
  `--listen`으로 명시 override할 수 있습니다.
- `render_manifest.json`은 deck-relative slash path를 기록하고, CLI가 현재 OS의
  native path로 해석해 Windows/Linux/macOS 간 path separator 차이를 줄입니다.
- plugin doctor helper는 Unix shell용 `scripts/slidex-doctor.sh`, Windows
  PowerShell용 `scripts/slidex-doctor.ps1`, Windows `cmd.exe`용
  `scripts/slidex-doctor.cmd`를 함께 제공합니다.
...
```bash
mise exec -- go test ./...
mise exec -- go run ./cmd/slidex validate-spec --spec examples/sample_deck_spec.json
mise exec -- go run ./cmd/slidex doctor --json
```
...
`doctor --render --json`으로 Chrome/Chromium 렌더 의존성을 먼저 확인한 뒤,
`SLIDEX_REQUIRE_RENDER_SMOKE=1`로 render/QA/package E2E가 skip되지 않도록 강제합니다.
나머지 Go 테스트는 `go test -json` 기반 annotation wrapper로 실행해 네이티브 runner
실패 원인을 test/package 단위로 남깁니다. 또한 `version`, `init`, `inspect --write`,
`migrate --dry-run`, `clean --logs`, `render`, `visual-review record`,
`qa --visual-review manual`, `finalize`, `review`, `package`,
`workbench save-smoke --screenshot`를 OS matrix에서 실제 CLI 명령으로 실행합니다.
`workbench save-smoke --screenshot`는 공백이 포함된 workspace path에서 실행해 subprocess
인자 전달과 loopback workbench screenshot 캡처를 검증합니다. Actions는 SHA로 고정합니다.
