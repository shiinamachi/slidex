...
새 deck creation을 Codex App에서 시작할 때는 plugin Solid Workbench를 사용합니다.
...
Workbench는 로컬 Solid Workbench로 표시되며, 사용자가 `Complete & generate`를 선택하면
...
production/canary release 설치본에서는 `workbench start`가 Workbench를 열기 전에 새
verified release를 자동 확인/적용합니다. 업데이트가 적용되면 현재 thread에서는
Workbench를 열지 않고 Codex 재시작 또는 pending activation 안내를 반환합니다.
